...
## Nieuw Custom Post Type aanmaken

Gebruik deze instructies wanneer een nieuw Custom Post Type nodig is.
...
5. **Vraag of er een ACF field group bij moet** (en zo ja, volg dan ook de flex block instructies hieronder)
...
## Nieuw flex content blok aanmaken

Gebruik deze instructies wanneer een nieuw flex content blok nodig is.
...
### Voorbeeld — ACF layout definitie (toevoegen aan `layouts` in `app/Acf/PageContent.php`):
...
### Voorbeeld — Blade view (`resources/views/blocks/hero-banner.blade.php`):
...
### Voorbeeld — Render loop in pagina-template (bijv. `resources/views/page.blade.php`):
